...
namespace penta_pod::kin::base_twerk {
  ...
  BaseTwerkCmdPuplisher::BaseTwerkCmdPuplisher() : node_{rclcpp::Node::make_shared("base_twerk_cmd_publisher")}
  {
    ...
    base_pose_publisher_ = node_->create_publisher<PoseStamped>("null_space_pose", 10);
  ...
  void BaseTwerkCmdPuplisher::create_setpoint_service() {
    ...
    auto lambda
      ...
      RCLCPP_INFO(
        node_->get_logger(), "Base pose setpoint received x: %f | y: %f | z: %f", 
    ...
    setpoint_service_ = node_->create_service<BasePoseSetpoint>("cmd_null_setpoint", lambda, rmw_qos_profile_services_default, callback_group_);
  }
  ...
  void BaseTwerkCmdPuplisher::timer_callback(){
    ...
    auto pose_optional = interpolate_pose(
      node_, 
      setpoint_base_pose_, 
      base_pose_, 
