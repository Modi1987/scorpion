...
BaseTwerkActionServer::BaseTwerkActionServer() : node_{rclcpp::Node::make_shared("base_twerk_action_server")} {
  ...
  this->base_pose_subscriber_ = node_->create_subscription<PoseStamped>(
      "null_space_pose", 10,
      [this](const PoseStamped& msg) -> void {
        {
  ...
  this->base_twerk_action_server_ = rclcpp_action::create_server<BaseTwerkAction>(
      node_,
      "base_twerk_action",
...
auto BaseTwerkActionServer::handle_goal(const rclcpp_action::GoalUUID&, std::shared_ptr<const BaseTwerkAction::Goal>)
    -> rclcpp_action::GoalResponse {
  RCLCPP_INFO_STREAM(node_->get_logger(), "Received a request to twerk the base");
  return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

auto BaseTwerkActionServer::handle_cancel(const std::shared_ptr<GoalHandle>) -> rclcpp_action::CancelResponse {
  RCLCPP_INFO(node_->get_logger(), "Received request to cancel goal");
  return rclcpp_action::CancelResponse::ACCEPT;
}

auto BaseTwerkActionServer::handle_accepted(const std::shared_ptr<GoalHandle> goal_handle) -> void {
  std::thread{[this, goal_handle]() { return this->execute(goal_handle); }}.detach();
}

auto BaseTwerkActionServer::execute(const std::shared_ptr<GoalHandle> goal_handle) -> void {
  ...
  auto is_timed_out = [this](rclcpp::Time start_time, std::chrono::milliseconds timeout_duration_millis) -> bool {
    auto elapsed_time = node_->now() - start_time;
    auto timeout_duration_sec = std::chrono::duration<double>(std::chrono::milliseconds(timeout_duration_millis)).count();
  ...
  PoseStamped start_pose{};
  {
    ...
    RCLCPP_INFO(node_->get_logger(), "Current base pose is: x:%f y:%f z:%f", start_pose.pose.position.x, start_pose.pose.position.y, start_pose.pose.position.z);
  }
  ...
  while (rclcpp::ok()) {
    ...
    auto current_pose = calculate_twerk_pose_from_goal(start_time, start_pose, goal_handle);
...
auto BaseTwerkActionServer::calculate_twerk_pose_from_goal(
  ...
  pose.pose.position.x = start_pose.pose.position.x + goal->rx * sin(goal->w * delta_t_seconds + goal->phi_x);
  pose.pose.position.y = start_pose.pose.position.y + goal->ry * sin(goal->w * delta_t_seconds + goal->phi_y);
  pose.pose.position.z = start_pose.pose.position.z + goal->rz * sin(goal->w * delta_t_seconds + goal->phi_z);

  return pose;
}


auto BaseTwerkActionServer::call_setpoint_client(const PoseStamped& base_to_basefootprint) -> bool {
  ...
  if (!service_call_template<BasePoseSetpointSrv>(node_, setpoint_client_, request, SERVICE_TIMEOUT_MILLIS)) {
...
auto BaseTwerkActionServer::get_parameters() -> void {
  ...
  load_param("base_twerk.action_server_service_call_interval_millis", update_interval_millis_int_,
