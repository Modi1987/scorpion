...
def generate_launch_description():
    ...
    ld.add_action(penta_core_launch)

    # Launch nav2 in simulation mode
    penta_nav2_sim_full = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                get_package_share_directory("penta_pod"),
                "launch",
    ...
    ld.add_action(penta_nav2_sim_full)
    ...
    gazebo_world_path = os.path.join(world_pkg, "worlds", "penta_gazebo_world.world")
    ...
    ros_gz_sim = IncludeLaunchDescription(
        ...
        launch_arguments={
            "gz_args": ["-r -v4 ", gazebo_world_path],
    ...
    gz_bridge_params_path = os.path.join(
        get_package_share_directory("penta_gazebo_sim"),
        "config",
        "ros_gz_bridge_laser_scan.yaml",
    )
    gz_bridge_node = Node(
    ...
    laser_scan_remapper_node = Node(
        ...
        output="screen",
        parameters=[{"name_space": LaunchConfiguration("name_space")}],
        remappings=[
